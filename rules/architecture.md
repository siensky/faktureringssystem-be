...
2. **Endast ägande tjänst läser sina tabeller.** Behöver du data från en annan tjänst: anropa dess API eller lyssna på dess event. Aldrig en `JOIN` över gränsen. **Ett dokumenterat, kvarstående undantag:** billing läser auths `tenants.status` direkt (`isTenantActive`, `listActiveTenantIds`) — den riktiga lösningen kräver en avstängningsendpoint och en operatörsroll som inte finns i någon planerad fas ännu (se kommentarerna vid de två anropen). **Ett andra, litet undantag (fas 9):** `users.customer_id REFERENCES customers (id) ON DELETE CASCADE` (`migrations/0009_portal.js`) — en tjänstegränsöverskridande FK, samma mönster som `customers.tenant_id -> tenants`. Konsekvensen: raderar billing en kund utan fakturor (domain.md #21) kaskaderar det automatiskt in i auths `users`-tabell, utanför auths egen applikationskod. Ingen ny `GRANT` behövs (Postgres FK-kontroller kör oavsett den infogande/raderande rollens egna `SELECT`-rättigheter), men det är fortfarande en koppling mellan tjänsternas data som inte går via ett API eller event. Accepterat medvetet — enkelhet väger tyngre än att bygga ett `customer.deleted`-event för ett fall som redan är sällsynt (en kund utan fakturor). **Ett tredje undantag (fas 12):** `user_company_links.customer_id REFERENCES customers (id) ON DELETE CASCADE` (`migrations/0011_bankid_customer_portal.js`) — samma familj av undantag, av samma skäl: en BankID-inloggningsidentitet (auth) kan vara länkad till flera kundrader (billing) samtidigt, och den kopplingen uttrycks enklast som en FK över tjänstegränsen snarare än ett event för varje förändring.
