...
// ska kunna existera, inte en genväg (code-style.md #21).
...
/**
 * Mock. `personalNumber` styr utfallet i collect():
 *   "PENDING" -> alltid pending
 *   "FAILED"  -> failed
...
export class MockBankIdProvider implements BankIdProvider {
  ...
  async collect(orderRef: string): Promise<CollectResult> {
    ...
    if (personalNumber === "PENDING") return { status: "pending", hintCode: "userSign" };
    if (personalNumber === "FAILED") {
